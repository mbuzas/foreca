...
const Location = () => {
    const { selectedLocation, locationName, dbUrl, getRandom } = useContext(WeatherContext);
    ...
    const getCurrentData = () => {
        ...
        selectedLocation && axios.request(currentOptions)
            .then(function (response) {
                ...
                postCurrentLog(response.data.current);
            })
            .catch(function (error) {
                console.error(error);
            });
    ...
    const postCurrentLog = async (data) => {
        await axios.post(dbUrl + "/selectedConditions", { "locationDetails": data })
            .then(function (response) {
            ...
            {dailyData &&
                <div className="daily-grid">
                    {dailyData.forecast.slice(0, 7).map((day) => {
                        return (
                            <div key={getRandom()} className="daily-wrapper">
                                <div className="block-mb" >
                                    <div className="content">
                                        <p>{day.date}</p>
                                        <p>Day: {day.maxTemp}</p>
